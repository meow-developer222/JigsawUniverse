...
function mouseDownInFullMode(e) {
...
function mouseDownInCellMode(e) {
    ...
    // for (let i=0; i < Board.instance.pieces.length; i++)
    {
        ...
        if (mx >= piece.x + SPINE_HEIGHT &&
            ...
            if (Board.instance.isNetwork)
                window.onPiecePick(piece.pieceID);
            break;
        }
    }
...
function mouseMoveInCellMode(e) {
    ...
    if (CellRenderer.instance.mySelectedPiece != -1) {
        ...
        if (Board.instance.isNetwork)
            window.onPieceDrag(mySelectedPiece.pieceID, mySelectedPiece.x, mySelectedPiece.y);
    }
}
