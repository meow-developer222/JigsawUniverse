import { MAX_OFFSET, PIECE_SIZE } from "./const.js";
import { LocalPuzzleDataManager } from "./datamgr.js";
...
export class Board {
    ...
    pieces = [];
    isNetwork = false;
    dataManager = new LocalPuzzleDataManager("workspace1");
    cellX = -1;
    cellY = -1;
    constructor(image, boardWidth, boardHeight, cellSize) {
    ...
    generatePieces() {
        ...
        for (let y = 0; y < this.boardHeight; y++) {
            for (let x = 0; x < this.boardWidth; x++) {
                ...
                let piece = new Piece((x % Board.instance.cellSize) * (PIECE_SIZE) + PIECE_SIZE / 2, (y % Board.instance.cellSize) * (PIECE_SIZE) + PIECE_SIZE / 2, (pieceOrder[(y % this.cellSize) * this.cellSize + (x % this.cellSize)] % (2 * this.cellSize)) * PIECE_SIZE / 2, Math.floor(pieceOrder[(y % this.cellSize) * this.cellSize + (x % this.cellSize)] / (2 * this.cellSize)) * PIECE_SIZE / 2, x, y, data.corners, data.spine_dir, 
                // x: x * (PIECE_SIZE) + 104 + (PIECE_SIZE) / 2,
                // y: y * (PIECE_SIZE) + 104 + PIECE_SIZE / 2,
                data.imageX, data.imageY);
