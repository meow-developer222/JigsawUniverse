...
function join() {
    socket.emit("join", document.getElementById("nicknameInput").value, window.location.pathname.split("/")[window.location.pathname.split("/").length - 1]);
...
socket.on("goRoom", (roomName) => {
    location.href = "/multi_play/" + roomName;
});
socket.on("players", (players_) => {
...
socket.on("select_piece", (playerID, pieceID) => {
...
socket.on("move_piece", (playerID, pieceID, x, y) => {
    if (pieceID === CellRenderer.instance.mySelectedPiece)
        return;
    let piece = Board.instance.pieces.find((piece) => piece.pieceID == pieceID);
    console.log(x, y);
...
socket.on("deselect_piece", (playerID, pieceID) => {
...
socket.on("correct", (playerID, pieceID) => {
    let piece = Board.instance.pieces.find((piece) => piece.pieceID == pieceID);
    piece.correct();
});
socket.on("board", (boardJSON) => {
    let board = JSON.parse(boardJSON);
    let pieces = board.pieces.map((pieceJSON) => Piece.fromJson(pieceJSON));
    const image = new Image();
    image.src = board.image_url;
    image.onload = () => onImageLoaded(image, board, pieces);
});
function onImageLoaded(image, board, pieces) {
...
window.onPiecePick = (pieceID) => {
    console.log(pieceID);
    socket.emit("select_piece", pieceID);
};
window.onPieceUnPick = (pieceID) => {
    socket.emit("deselect_piece", pieceID);
};
window.onPieceDrag = (pieceID, x, y) => {
